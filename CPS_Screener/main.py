...
def screen_csp(
    ...
    rows = []        # filtered rows that pass all criteria
    raw_rows = []    # all option rows seen before tight filters
    today = date.today()
    ...
    for tkr in tickers:
        try:
            ...
            if not expirations:
                print(f"{tkr}: no options expirations returned, skipping")
                continue
            ...
            for exp in expirations:
                ...
                if puts.empty:
                    print(f"{tkr} {exp}: no puts in chain")
                    continue
                ...
                for _, opt in puts.iterrows():
                    ...
                    # Basic sanity: still record raw row even if filters fail later
                    T = dte / 365.0
                    delta = bs_put_delta(S, K, T, r, iv) if np.isfinite(iv) and iv > 0 else float("nan")
                    ...
                    # Below this point are the stricter filters used for final candidates
    ...
    # Always dump the raw collected option rows for inspection (even if filters remove everything)
    raw_df = pd.DataFrame(raw_rows)
    print(f"Collected raw_rows: {len(raw_rows)}")
    try:
        raw_df.to_csv("CPS_Screener\\csp_candidates_raw.csv", index=False)
        print("Saved raw candidates to CPS_Screener\\csp_candidates_raw.csv")
    except Exception as e:
        print("Error writing csp_candidates_raw.csv:", e)
...
if __name__ == "__main__":
    ...
    df = screen_csp(
        tickers=tickers,
        min_dte=1,           # loosened from 7
        max_dte=90,          # loosened from 45
        delta_low=-0.60,     # loosened from -0.30
        delta_high=-0.05,    # loosened from -0.20
        min_return=0.002,    # loosened from 0.005 (0.2% vs 0.5%)
        max_spread=0.30,     # loosened from 0.10
        min_cushion=0.0,     # loosened from 0.01
        r=0.045,
        hist_lookback="3mo", # shortened from 6mo
        # Optional liquidity constraints:
        # min_volume=50,
        # min_oi=200,
        only_otm=False,      # allow ATM/ITM as well
        top_n_per_ticker=10
    )
    ...
    if df.empty:
        ...
        # Save to CSV
        df.to_csv("CPS_Screener\\csp_candidates.csv", index=False)
        print("\nSaved: CPS_Screener\\csp_candidates.csv")
