...
def main():
    ...
    # Write backtest results to S3 and to /tmp for Learn_Adjustments
    s3_io.write_csv(bt, "backtest_results.csv")
    bt_path = "/tmp/backtest_results.csv"
    ...
    if not bt.empty:
        ...
        s3_io.write_csv(summary, "backtest_summary.csv")
        print(f"[pipeline] Backtest summary: home_mae={home_mae:.2f}, away_mae={away_mae:.2f}, n={len(bt)}")
        print(f"[pipeline] Wrote backtest_results.csv and backtest_summary.csv to S3")
    else:
    ...
    try:
        ...
        s3_io.write_csv(adj_df, "team_adjustments.csv")
        print(f"[pipeline] Wrote adjustments (team_adjustments.csv) to S3 (local path was {adj_path})")
    except Exception as e:
        print(f"[pipeline] Warning: failed to load/write team_adjustments from {adj_path}: {e}")

    # 4) Save tuned globals for downstream use
    try:
        params_path = "/tmp/model_adjustments.csv"
        ...
        s3_io.write_csv(params, "model_adjustments.csv")
        print(f"[pipeline] Updated tuned params in S3 (local path was {params_path})")
    except Exception:
        pass
    ...
    # 6) Email notification via SES with CSV attachment
    try:
        ...
        if sender and recipient and bucket:
            # Fetch CSV from S3
            s3 = boto3.client("s3")
            ...
            # Build MIME email
            msg = MIMEMultipart()
            msg["Subject"] = "NBA pipeline results: predicted_game_scores_today.csv"
            msg["From"] = sender
            msg["To"] = recipient
            ...
            part = MIMEApplication(csv_bytes)
            ...
            print(f"[pipeline] Sent SES email with attachment to {recipient}")
        else:
            print("[pipeline] SES email skipped (NBA_EMAIL_FROM, NBA_EMAIL_TO, or NBA_DATA_BUCKET not set).")
    except Exception as e:
        print(f"[pipeline] Warning: failed to send SES email with attachment: {e}")
