...
def _read_history(data_dir: str) -> pd.DataFrame:
    # Prefer consolidated history from S3 if present
    try:
        df = s3_io.read_csv("nba_game_scores.csv")
        return df
    except Exception:
        pass

    # Fallback: try local file (useful when running on your PC)
    local_path = os.path.join(data_dir, "nba_game_scores.csv")
    ...
    # Fallback: stitch from backtest cache games files (local)
...
def todays_games(data_dir: str) -> pd.DataFrame:
    if load_today_context is not None:
        try:
            ...
            if todays is not None and not todays.empty:
                return todays[["home_code", "visitor_code"]].rename(
                    columns={"home_code": "home", "visitor_code": "away"}
                ).astype(str)
        except Exception:
            pass

    # Fallback: infer today's games from S3 nba_game_scores.csv using Pacific date
    try:
        df = s3_io.read_csv("nba_game_scores.csv")
    except Exception:
    ...
    df["game_date_pst"] = df["game_date"].dt.tz_convert(pacific)
    today_pst = datetime.now(pacific).date()

    todays_df = df[df["game_date_pst"].dt.date == today_pst].copy()
    if todays_df.empty:
        return pd.DataFrame(columns=["home", "away"])

    if "home_code" in todays_df.columns and "visitor_code" in todays_df.columns:
        out = todays_df[["home_code", "visitor_code"]].rename(
            columns={"home_code": "home", "visitor_code": "away"}
        )
    elif {"home", "away"}.issubset(todays_df.columns):
        out = todays_df[["home", "away"]]
    else:
        return pd.DataFrame(columns=["home", "away"])
...
def predict_from_history(hist: pd.DataFrame, games_today: pd.DataFrame, last_n: int = 10, home_adv: float = 1.5, w_for: float = 0.5, use_decay: bool = False, half_life: float = 0.0) -> pd.DataFrame:
    ...
    avgs = (
        long2.groupby("team")[["pts", "pts_against"]]
        .apply(
    ...
    long2 = pd.concat([long2.reset_index(drop=True), avgs.reset_index(drop=True)], axis=1)
